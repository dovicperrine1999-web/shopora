...
-- =====================================================================
-- V61 PRODUCT STATUS COMPATIBILITY
-- Fix legacy databases whose products_status_check does not allow the
-- statuses used by Shopora (active / out_of_stock). This is idempotent.
-- Existing invalid legacy values are normalized before the constraint is
-- recreated so the migration does not fail on old data.
-- =====================================================================
do $$
begin
  if to_regclass('public.products') is not null then
    ...
    set status = case
      when coalesce(stock,0) > 0 then 'active'
      else 'out_of_stock'
    end
    where status is null
       or status not in ('draft','active','out_of_stock','archived');
...
begin
 ...
 msg=case p_status when 'processing' then 'Payment confirmed. The seller is preparing your package.'
   ...
   when 'shipped' then 'Your package has shipped.' when 'delivered' then 'Your package was marked delivered.'
