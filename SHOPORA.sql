...
as $$
  select
    ...
    coalesce(nullif(trim(pr.full_name),''), nullif(trim(u.raw_user_meta_data->>'full_name'),''), nullif(split_part(u.email,'@',1),''), 'Customer'),
    coalesce(nullif(trim(pr.phone),''), nullif(trim(u.raw_user_meta_data->>'phone'),''), ''),
