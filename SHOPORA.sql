...
declare existsrow boolean;
begin
 select exists(select 1 from public.wishlists where user_id=auth.uid() and product_id=p_product) into existsrow;
 if existsrow then delete from public.wishlists where user_id=auth.uid() and product_id=p_product; return false;
 else insert into public.wishlists(user_id,product_id) values(auth.uid(),p_product); return true; end if;
end $$;

-- V24: notify a seller when a customer adds one of their products to a wishlist.
create or replace function public.notify_shopora_wishlist_add()
returns trigger
language plpgsql
security definer
set search_path=public
...
begin
  select s.owner_id, p.name
    into seller_user, product_name
    from public.products p
    join public.shops s on s.id=p.shop_id
   where p.id=new.product_id;

  if seller_user is null or seller_user=new.user_id then
    return new;
  end if;

  select coalesce(nullif(trim(pr.full_name),''), 'A customer')
    into customer_name
  ...
  values(
    ...
    customer_name || ' added “' || coalesce(product_name,'your product') || '” to their wishlist.'
  );
...
drop trigger if exists trg_shopora_wishlist_add on public.wishlists;
create trigger trg_shopora_wishlist_add
after insert on public.wishlists
for each row execute function public.notify_shopora_wishlist_add();
