...
### V13 required database step
If the existing Supabase project still shows an error when clearing notifications, run `sql/03_notifications_clear.sql` once in the Supabase SQL Editor. The migration adds a user-scoped DELETE policy and recreates the clear function.
